    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # call the function to clear all of the textboxes and the slected index of listbox
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 ==-0: # if proposed changes are not already in the databese, then proceed
